...
class SNNIDESTUDENTBERTSpike(nn.Module):

    def __init__(self, cfg_path, load_pretrained = False, t_conv = 100, vth = 1., **kwargs):
        super(SNNIDESTUDENTBERTSpike, self).__init__()
        ...
        self.vth = vth
        self.dropout = 0.0
        self.leaky = .99 #1 means IF neuron; 0<self.leaky<1 mean LIF
        self.solver = 'broy'
        ...
        # Feedback: optional
        self.network_s13 = SNNFC(config.hidden_size,config.hidden_size, dropout = 0.1)
    ...
    def _forward(self, input_ids, segment_ids, attention_mask, **kwargs):
        ...
        student_rep, attn_val = self.snn_ide_conv(input_ids, segment_ids, attention_mask = attention_mask, time_step=time_step, threshold=threshold, input_type=input_type, solver_type=self.solver, leaky=leaky)

        return student_rep, attn_val

    def forward(self, input_ids, segment_ids, attention_mask, **kwargs):
        student_rep, attn_val = self._forward(input_ids, segment_ids, attention_mask, **kwargs)
        tmp = []
