...
#from implicit_bert.models.snn_ide_bert_spike import SNNIDESTUDENTBERTSpike
from implicit_bert.models.snn_ide_bert_spike_4 import SNNIDESTUDENTBERT4Spike
    ...
    def __init__(self, training_path, epoch, tokenizer, num_data_epochs, reduce_memory=False):
        ...
        if reduce_memory:
            ...
            is_nexts = np.memmap(filename=self.working_dir/'is_nexts.memmap',
                                 shape=(num_samples,), mode='w+', dtype=np.bool_)
        else:
            input_ids = np.zeros(shape=(num_samples, seq_len), dtype=np.int32)
            input_masks = np.zeros(shape=(num_samples, seq_len), dtype=np.bool_)
            segment_ids = np.zeros(shape=(num_samples, seq_len), dtype=np.bool_)
            lm_label_ids = np.full(shape=(num_samples, seq_len), dtype=np.int32, fill_value=-1)
            is_nexts = np.zeros(shape=(num_samples,), dtype=np.bool_)
...
def main():
    ...
    if args.continue_train:
        #student_model = SNNIDESTUDENTBERT(args.spiking_student_model, load_pretrained=True)
        student_model = SNNIDESTUDENTBERT4Spike(args.spiking_student_model, t_conv=args.t_conv, vth=args.vth, load_pretrained=True)
    else:
        #student_model = SNNIDESTUDENTBERT(args.spiking_student_model, load_pretrained=False)
        student_model = SNNIDESTUDENTBERT4Spike(args.spiking_student_model, t_conv=args.t_conv, vth=args.vth, load_pretrained=False)
    ...
    #optimizer = torch.optim.Adam(student_model.parameters(), lr=0.0001, weight_decay=args.weight_decay, betas=(0.9, 0.999))
    global_step = 0
    ...
    for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        train_dataloader = DataLoader(epoch_dataset, batch_size=args.train_batch_size)
        ...
        with tqdm(total=len(train_dataloader), desc="Epoch {}".format(epoch)) as pbar:
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", ascii=True)):
                ...
                # with torch.no_grad():
                #     input_embedding = teacher_model.module.embeddings(input_ids, segment_ids)

                student_reps, student_atts = student_model(input_ids, segment_ids, input_mask)
